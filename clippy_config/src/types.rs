...
test_lint = "allow"
"#;
        let table: std::collections::BTreeMap<toml::Spanned<String>, toml::Spanned<LintConfig>> = toml::from_str(simple_toml).unwrap();
...
"#;
        let table: std::collections::BTreeMap<toml::Spanned<String>, toml::Spanned<LintConfig>> = toml::from_str(table_toml).unwrap();
        let config = table["test_lint"].get_ref();
...
"#;
        let table: std::collections::BTreeMap<toml::Spanned<String>, toml::Spanned<LintConfig>> = toml::from_str(table_no_priority_toml).unwrap();
        let config = table["test_lint"].get_ref();
